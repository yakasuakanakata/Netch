    ...
    public static class NetCardInfoTool
    {
            ...
            try
            {
                ...
                foreach (var item in adapters)
                {
                        ...
                        item.NetworkInterfaceType == NetworkInterfaceType.Wireless80211)
                    {
                        ...
                        //测试获取数据
                        var ip = item.GetIPProperties().UnicastAddresses.Count >= 1
                            ? item.GetIPProperties().UnicastAddresses[0].Address.ToString()
                            : null;
                        //更新IP为ipv4地址
                        if (item.GetIPProperties().UnicastAddresses.Count > 0)
                        {
                            ip = item.GetIPProperties()
                                .UnicastAddresses[item.GetIPProperties().UnicastAddresses.Count - 1].Address.ToString();
                        }

                        var gateway = item.GetIPProperties().GatewayAddresses.Count >= 1
                            ? item.GetIPProperties().GatewayAddresses[0].Address.ToString()
                            : null;
        ...
        public static List<string> GetAllIPv4Address()
        {
            ...
            try
            {
                ...
                if (ListTool.HasElements(temp))
                {
                    hosts.AddRange(from t in temp
                        where t.AddressFamily == AddressFamily.InterNetwork
                        select t.ToString());
                }
            }
            catch (Exception)
